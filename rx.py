...
def receive_loop():
    ...
    while True:
        if GPIO.input(DIO0):
            ...
            if irq_flags & 0x40:  # Check if RX done flag is set
                spi_write(0x12, 0xFF)  # Clear IRQ flags
                nb_bytes = spi_read(0x13)  # Get payload length
                current_addr = spi_read(0x10)
                spi_write(0x0D, current_addr)  # Set FIFO pointer
                ...
                # Add a very short delay after processing the packet
                time.sleep(0.005)
